...
- 临时措施必须记录撤销条件与正式化条件
- 编码前思考：即使紧急，也必须先确认故障目标、影响范围、止血目标、回滚条件和最小验证路径
- 简洁优先：优先恢复服务和控制风险，不在热修中引入复杂重构或长期架构调整
- 精准修改：只修改止血所需的最小代码、配置或数据对象
- 目标驱动执行：以服务恢复、最小验证、风险可控、临时措施留痕和主链回流作为完成标准
- 可按需使用 GitNexus 类代码知识图谱辅助定位故障入口、调用链、最小修复点和冒烟路径
- 可按需使用 graphify 类能力辅助形成事故关系图或 postmortem 导读，但不得替代事件报告和验证证据
...
flowchart TB
    ...
    subgraph S1 ["Step 1: 确认故障目标与影响范围"]
        STEP1["收集故障描述 现场症状<br>识别影响范围与严重程度<br>确定修复优先级与目标<br>按需使用 GitNexus 辅助定位调用链<br>评估是否需要紧急回滚"]
    end
    ...
    subgraph S4 ["Step 4: 记录临时措施与回流动作"]
        STEP4["记录临时措施内容 适用范围<br>记录撤销条件与正式化条件<br>评估回流到各阶段的需要<br>按需补充 graphify 事故关系导读"]
    end
