...
flowchart TB
    ...
    PRECHECK -->|知识库未初始化| ABORT_INIT[⚠️ 请先执行初始化<br/>/mes-init-project]
    PRECHECK -->|契约知识不可消费| ABORT_CONTRACT[⚠️ 先确认 contracts.md<br/>可用]
    ...
    subgraph STEP1_PHASE["Step 1：需求解析"]
        STEP1[mes-analyze-parse-requirement<br/>━━━━━━━━━━━━━━━━<br/>将原始业务语言<br/>解析为结构化要素] --> STEP1_OUT[产出：结构化需求要素<br/>业务目标 / 功能范围<br/>约束条件 / 验收标准]
    end
    ...
    subgraph STEP2_PHASE["Step 2：影响范围识别"]
        STEP2[mes-analyze-identify-repos<br/>━━━━━━━━━━━━━━━━<br/>识别涉及的代码仓<br/>数据库 Schema 与配置] --> STEP3[mes-analyze-impact-scope<br/>━━━━━━━━━━━━━━━━<br/>分析影响的服务/模块/表]
    end

    STEP3 --> STEP2_OUT[产出：影响范围结论<br/>受影响仓 / 服务 / 模块 / 表<br/>风险等级 / 依赖关系]
    ...
    subgraph STEP3_PHASE["Step 3：仓级责任边界与 Provider 决策"]
        STEP4[仓级责任边界形成<br/>━━━━━━━━━━━━━━━━<br/>区分候选仓 / 确认仓 / 待补证仓] --> STEP5[Provider 选择判断<br/>━━━━━━━━━━━━━━━━<br/>技术可达 / 架构允许 / 最终选定]
        STEP5 --> STEP6[API 复用判断<br/>━━━━━━━━━━━━━━━━<br/>复用 / 扩展 / 新增]
    end

    STEP6 --> STEP3_OUT[产出：仓级责任边界<br/>Provider 选择理由<br/>API 复用结论]
    ...
    subgraph STEP4_PHASE["Step 4：业务流追踪"]
        STEP7[mes-analyze-trace-flow<br/>━━━━━━━━━━━━━━━━<br/>追踪跨服务调用链<br/>验证数据流向与依赖] --> STEP7_OUT[产出：业务调用链<br/>数据流向 / 异常路径<br/>关键依赖节点]
    end
    ...
    subgraph STEP5_PHASE["Step 5：规格文档生成"]
        STEP8[mes-analyze-generate-spec<br/>━━━━━━━━━━━━━━━━<br/>生成 OpenSpec 需求规格<br/>整合前序分析结论] --> STEP8_OUT[产出：spec.md<br/>需求规格文档]
    end
    ...
    subgraph STEP6_PHASE["Step 6：需求评审"]
        STEP9[mes-analyze-review-spec<br/>━━━━━━━━━━━━━━━━<br/>审查完整性 / 一致性<br/>可测试性 / 可实现性] --> STEP9_OUT[产出：spec-review-report.md<br/>详细审查报告]
    end

    STEP9_OUT --> GATE_EXIT{退出门禁}

    GATE_EXIT -->|❌不通过| STEP1
    GATE_EXIT -->|✅通过| SWEEP[Completion Sweep<br/>━━━━━━━━━━━━━━━━<br/>设计可以继续吗？<br/>最小决策点已压缩？<br/>阻断歧义已区分？]

    SWEEP --> FINAL[最终输出<br/>━━━━━━━━━━━━━━━━<br/>需求目标 + 影响范围<br/>+ 核心验收标准<br/>+ 关键歧义点 + blocker<br/>+ 风险说明 + 推荐下一步]

    FINAL --> DONE([✅ 需求分析完成<br/>可进入设计阶段])
...
```mermaid
flowchart TB
    REQ[需求涉及多服务/多仓] --> SCAN[扫描知识库<br/>backend-overview / dependencies]

    SCAN --> IDENTIFY[识别候选仓]
    IDENTIFY --> C1[确认仓<br/>已验证直接涉及]
    IDENTIFY --> C2[候选仓<br/>可能涉及，待确认]
    IDENTIFY --> C3[待补证仓<br/>影响不确定]
    ...
    PROVIDER --> P1[技术可达 Provider<br/>技术栈支持]
    PROVIDER --> P2[架构允许 Provider<br/>架构约束允许]
    PROVIDER --> P3[最终选定 Provider<br/>综合考量确定]
...
flowchart TB
    ...
    CHECK -->|全部满足| GSD[🚀 GSD 模式<br/>最小需求包快速推进]
    CHECK -->|任一不满足| STRICT[🔒 Strict 模式<br/>完整治理链]

    GSD --> MIN_PKG[最小需求包<br/>目标 + 影响初判<br/>+ 核心验收 + 歧义点<br/>+ blocker + 风险]
    ...
    STRICT --> FULL_ANALYSIS[完整分析流程<br/>六步骤全部执行]
...
├── handoff/
│   └── analyze-to-design-handoff.md  # 分析→设计交接
└── working/
