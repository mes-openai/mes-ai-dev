...
related_files:
  - knowledge/rules/skeleton-change-governance-summary.md
  - workspace/refresh/skeleton-change-log.md
---

# 骨架修改治理规则

> 对应摘要：`knowledge/rules/skeleton-change-governance-summary.md`
...
- `mes-ai-dev/templates/` 下的模板与模板索引
...
7. 骨架应在满足治理完整性的前提下尽可能简洁；新增规则、模板、说明、示例时，应优先复用现有机制，避免无必要膨胀。
8. 骨架必须适配大需求场景下的上下文控制，设计时应优先保证 AI 知道“该读什么、不该读什么”，避免无边界读取导致上下文爆掉。
9. 骨架必须支持复用到其它项目；新增骨架能力时，应优先考虑是否能沉淀为可迁移、可参数化、可编排的标准能力，而非仅对当前项目有效。
10. 骨架工作目录必须与业务代码仓目录隔离，骨架治理规则不得弱化这一边界。
...
主要来源：`AGENTS.md`、`knowledge/reference/phase-gates.md`、`knowledge/rules/artifact-standards.md`
...
3. 必须在以下位置同步至少一处可发现入口：
   ...
   - `mes-ai-dev/knowledge/reference/team-onboarding-guide.md` 的 Command 速查表或使用流程
...
主要来源：`AGENTS.md`、`knowledge/reference/team-onboarding-guide.md`、`操作地图-飞行员手册.md`
...
5. 若本次骨架修改涉及**新增、修改、删除标准产物、目录职责、标准文件名、模板对应产物或产物分类**，必须同步刷新：
   ...
   - `mes-ai-dev/knowledge/reference/command-skill-artifact-map.md`（若命中命名或映射变更）
   - 相关入口索引、模板索引与结构说明文件
6. 若上述同步刷新未完成，则本次骨架修改不得判定为完成。

主要来源：`AGENTS.md`、`workspace/refresh/skeleton-change-log.md`
...
主要来源：`AGENTS.md`、`knowledge/rules/state-rendering-spec.md`、`knowledge/reference/knowledge-consumption-matrix.md`
...
主要来源：`AGENTS.md`、`knowledge/rules/path-canonicalization-rules.md`、`knowledge/rules/state-rendering-spec.md`
...
主要来源：`AGENTS.md`、`knowledge/rules/state-rendering-spec.md`
...
5. 若骨架修改涉及 `.opencode/commands/`、`.opencode/skills/`、阶段标准产物文件名或阶段评审报告命名，必须同步检查并按需更新 `mes-ai-dev/knowledge/reference/command-skill-artifact-map.md`。
6. 若 `command-skill-artifact-map.md` 已更新，必须同步检查 README、模板索引、团队接入规范、飞行员手册等入口文件是否仍与最新映射一致。
7. 若骨架修改涉及“哪些产物由 AI 生成 / 人补充 / 人主导”的边界变化，必须同步更新 `mes-ai-dev/knowledge/reference/skeleton-artifact-ownership-guide.md`。
8. 若骨架修改涉及“阶段产物种类、分类、优先级、阅读方式”的变化，必须同步更新 `mes-ai-dev/knowledge/reference/stage-artifact-guide.md`。
9. 若新增文档属于面向人阅读的长期入口文档，必须至少同步检查：
   - `mes-ai-dev/knowledge/reference/skeleton-loading-matrix.md`
   - `mes-ai-dev/knowledge/reference/team-onboarding-guide.md`
   - `mes-ai-dev/knowledge/reference/knowledge-structure.md`
   - `mes-ai-dev/knowledge/reference/workspace-structure.md`
   - `mes-ai-dev/templates/template-index.md`
10. 若骨架修改涉及 Skill 目录结构、Skill 读取顺序、模块拆分规则或 evals 结构，必须同步检查并按需更新：
   - `AGENTS.md`
   - `mes-ai-dev/knowledge/reference/phase-skill-index.md`
   - `mes-ai-dev/knowledge/reference/skeleton-loading-matrix.md`
   - `mes-ai-dev/knowledge/reference/team-onboarding-guide.md`
   - `mes-ai-dev/knowledge/reference/knowledge-structure.md`
...
主要来源：`AGENTS.md`、`templates/template-index.md`、`knowledge/reference/knowledge-consumption-matrix.md`

### 3.8 历史文档阅读边界约束

1. `workspace/refresh/` 下历史审计、复盘、说明文档反映的是生成当时的骨架状态。
2. 历史文档不得作为当前骨架规则的唯一依据。
3. 历史口径与现行规则冲突时，以最新 `AGENTS.md`、主规则文件、消费矩阵、门禁标准为准。

主要来源：`workspace/refresh/README.md`
...
4. 若全局复盘发现可复发的治理问题，必须同步评估是否更新 `knowledge/reference/governance-memory.md`。
5. 未完成全局复盘或未给出建议方案，不得判定本次骨架约束变更已完成。

主要来源：`knowledge/reference/phase-gates.md`、`workspace/refresh/skeleton-change-review-20260416-骨架治理闭环首件.md`
...
主要来源：`AGENTS.md`、`knowledge/rules/budget-audit-rules.md`、`knowledge/reference/knowledge-consumption-matrix.md`
...
主要来源：`AGENTS.md`、`workspace/refresh/README.md`、`templates/template-index.md`
...
主要来源：`AGENTS.md`、`knowledge/rules/scenarios/scenario-gsd.md`、`knowledge/reference/blocker-handling-guide.md`、`knowledge/reference/dod-definition-guide.md`
...
9. 自动调优的规则、判定、流程、状态模型与审计载体，统一以下列文件为准：
   ...
   - `mes-ai-dev/knowledge/reference/document-auto-optimization-flow.md`
   - `mes-ai-dev/knowledge/reference/document-auto-optimization-state-model.md`
   - `mes-ai-dev/templates/governance/document-auto-optimization-audit-template.md`
...
6. 若新增约束涉及阶段治理、审查机制或状态追踪，还必须评估是否需要同步更新：
   - `knowledge/reference/governance-memory.md`
   - `knowledge/reference/knowledge-consumption-matrix.md`
   - 对应说明入口或示例文件
...
1. 骨架修改不属于普通业务需求交付物。
2. 骨架治理、规则、审计、复盘、说明与日志已集中在 `workspace/refresh/` 下。
...
3. 若仅用于演示模板成品形态，应写入 `workspace/examples/`，不得冒充真实审查结果。
...
1. 骨架修改审查文件应优先使用：`mes-ai-dev/templates/governance/skeleton-change-review-template.md`
...
- 总入口：`AGENTS.md`
- 门禁标准：`mes-ai-dev/knowledge/reference/phase-gates/index.md`
- 知识消费：`mes-ai-dev/knowledge/reference/knowledge-consumption/index.md`
- 状态规范：`mes-ai-dev/knowledge/rules/state/state-rendering-index.md`
- 路径规范：`mes-ai-dev/knowledge/rules/path-canonicalization-rules.md`
- 产物限制：`mes-ai-dev/knowledge/rules/artifact-standards.md`
- 模板导航：`mes-ai-dev/templates/template-index.md`
- 骨架修改审查模板：`mes-ai-dev/templates/governance/skeleton-change-review-template.md`
- 骨架约束摘要：`mes-ai-dev/knowledge/reference/skeleton-constraint-summary.md`
