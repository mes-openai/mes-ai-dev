...
related_files:
  - knowledge/rules/artifact-standards-summary.md
  - knowledge/rules/governance/stage-artifact-layout.md
---

# 产物压缩规范

> 对应摘要：`knowledge/rules/artifact-standards-summary.md`
...
各阶段 `REQ-YYYYMMDD-XXX` 目录必须采用以下标准分类目录组织产物：
...
- 阶段完成产物报告：`report/stage-output-report.md`
- 阶段主交接文档：符合 OpenSpec 的主文档
- 坑点台账：`memory/pitfall-ledger.md`
- 决策日志：`memory/decision-log.md`
- 阻塞台账：`memory/blocker-log.md`
...
|---------|---------|---------------|------|
| requirement-spec.md | 200 行 | ~3K token | 需求规格 |
| impact-scope.md | 150 行 | ~2K token | 影响范围 |
| design-doc.md | 500 行 | ~8K token | 详细设计 |
...
1. **文档超过最大行数** → 必须压缩：
   - 将详细内容拆分到附录文件
   - 主文件只保留摘要 + 引用路径
   - 例如：design-doc.md 超过 500 行 → 拆分为 design-doc.md（摘要）+ mes-design-api-detail.md（API详情）
...
归档时在 `workspace/archive/REQ-XXX/summary.md` 生成压缩摘要（≤500 token），包含：
- 需求名称和编号
- 涉及服务列表
- 关键变更摘要
- 交付结论
...
5. 阶段完成产物报告必须引用阶段交接产物位置，至少包含当前阶段 OpenSpec 主交接文档与 `memory/*` 的产出状态
...
| 文件类型 | 模板 | 用途 |
|---------|------|------|
| `*-completion.md` | `mes-ai-dev/templates/develop/agent-handoff-template.md` | 单个 Agent 完成某任务单元后的交接 |
| `*-results.md` | `mes-ai-dev/templates/develop/agent-handoff-template.md` | 单个 Agent 分析/测试/审核结果交接 |
| 步骤级门禁审查记录 | `mes-ai-dev/templates/governance/step-gate-review-template.md` | 单步骤输出进入下一步骤前的门禁审查记录 |
