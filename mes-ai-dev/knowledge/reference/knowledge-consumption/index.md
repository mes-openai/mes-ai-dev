...
> 本目录用于替代原 `knowledge/reference/knowledge-consumption-matrix.md` 的单文件重型结构。
...
补充说明：上表中的 `stage-memory.md` 指 `knowledge/reference/knowledge-consumption/stage-memory.md` 这一消费分片，不得与阶段目录下的 OpenSpec 主交接文档混淆。
...
2. index：`knowledge/reference/parameter-switch-patterns.md`、`knowledge/reference/terminology-glossary.md` 与当前主题索引
...
原 `knowledge/reference/knowledge-consumption-matrix.md` 仍保留为兼容导航入口。
