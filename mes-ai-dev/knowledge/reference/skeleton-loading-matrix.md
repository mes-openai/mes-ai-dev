...
2. 不再把 `phase-gates.md`、`knowledge-consumption-matrix.md` 作为默认整份必读入口；应优先使用：
   ...
   - `knowledge/reference/phase-gates/<当前阶段>.md`
   - `knowledge/reference/knowledge-consumption/index.md`
   - `knowledge/reference/knowledge-consumption/<当前主题>.md`
3. 非当前阶段门禁分片默认不加载。
4. 非当前问题范围的消费矩阵主题分片默认不加载。
5. 场景规则只在命中特殊条件时加载，不得因为“保险起见”默认全文常驻。
6. 骨架文档若嵌入加载元信息，优先读取元信息块判断是否需加载正文；元信息格式见 `knowledge/rules/governance/document-load-metadata-standard.md`。
7. 若关键定义位于 SDK / common / shared / integration / 外部依赖中，应优先加载 `knowledge/reference/knowledge-consumption/contracts.md`，再按需进入 `knowledge/rules/scenarios/scenario-external-contract-source.md`，不得直接从业务接口表象反推契约。
...
- `index` / `common` 指 `knowledge/reference/phase-gates/` 目录下的总索引与通用门禁分片
- `state` / `rules` / `reference` 等指 `knowledge/reference/knowledge-consumption/` 目录下的主题分片
- `parameter-switch-patterns` 指 `knowledge/reference/parameter-switch-patterns.md`，用于参数开关三种模式、复用优先级与取值来源判断
- `contracts` 指 `knowledge/reference/knowledge-consumption/contracts.md`，用于统一响应、错误码、SDK 模型、认证/MQ 等契约级知识消费
...
1. 运行入口文件（`knowledge/rules/core/runtime-entry.md`）
2. Core 常驻规则
3. 当前阶段规则
...
- 命中参数开关场景时必须读取 `knowledge/reference/parameter-switch-patterns.md`
...
- `knowledge/rules/artifact-standards.md`
- `knowledge/reference/dod-definition-guide.md`
- `knowledge/reference/blocker-handling-guide.md`
- `knowledge/reference/team-onboarding-guide.md`
- `knowledge/reference/stage-memory-governance.md`
- `knowledge/reference/status-tracker-governance.md`
- `templates/**/*.md` 正文
- 非当前阶段门禁分片
- 非当前问题范围的知识消费分片
...
- `knowledge/reference/phase-skill-index.md`
- `templates/template-index.md`
- `knowledge/reference/exception-handbook.md`
- `knowledge/reference/command-skill-artifact-map.md`
- `knowledge/reference/skeleton-maintainer-quick-reference.md`
- `knowledge/reference/skeleton-artifact-ownership-guide.md`
- `knowledge/reference/stage-artifact-guide.md`
...
- `knowledge/reference/phase-gates.md`
...
分层加载矩阵定义静态规则层级，运行时决策由 `knowledge/rules/core/runtime-entry.md` 补充，用于判断：
...
- 上下文预算基线：`mes-ai-dev/knowledge/rules/context-budget-baseline.md`
- 规则主定义矩阵：`mes-ai-dev/knowledge/reference/rule-authority-matrix.md`
- 骨架维护速查：`mes-ai-dev/knowledge/reference/skeleton-maintainer-quick-reference.md`
- 骨架产物责任清单：`mes-ai-dev/knowledge/reference/skeleton-artifact-ownership-guide.md`
- 阶段产物说明：`mes-ai-dev/knowledge/reference/stage-artifact-guide.md`
