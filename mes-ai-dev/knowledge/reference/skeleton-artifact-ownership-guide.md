...
> 适用范围：`AGENTS.md`、`.opencode/commands/`、`.opencode/skills/`、`mes-ai-dev/knowledge/`、`mes-ai-dev/templates/`、`mes-ai-dev/workspace/examples/`、`mes-ai-dev/workspace/refresh/` 下的骨架文件。  
> 配套规则：`mes-ai-dev/knowledge/rules/skeleton-change-governance.md`
...
| `knowledge/rules/` | 硬规则、阶段规则、治理规则 | 人主导，AI 辅助 | 人定规则，AI 帮收敛与互链 | 不允许双重口径 |
| `knowledge/reference/` | 面向人阅读的说明、索引、手册 | AI 主生成 + 人补充 | 先 AI 生成结构化说明，再人工补经验与判断 | 本次新增文档主要落在这里 |
| `templates/` | 标准模板 | AI 主生成 + 人补充 | AI 起草模板结构，人补字段解释与适用边界 | 需进 `template-index.md` |
| `workspace/examples/` | 示例产物 | AI 主生成 + 人补充 | AI 造样例，人校对是否贴近真实流程 | 不得替代真实规则 |
| `workspace/refresh/` | 变更留痕、审查、回归、刷新报告 | AI 主生成 + 人确认 | AI 写日志报告初稿，人确认影响和结论 | 必须可追溯 |

### 4.2 目录优先级建议

| 优先级 | 目录 | 理由 |
|---|---|---|
| 高 | `AGENTS.md`、`knowledge/rules/` | 会直接改变骨架执行方式 |
| 高 | `knowledge/reference/` 核心 guide / index | 会影响人如何理解和使用骨架 |
| 中 | `templates/` | 会影响标准产物长相和字段完整性 |
| 中 | `.opencode/commands/`、`.opencode/skills/` | 会影响执行路径与产物生成 |
| 中 | `workspace/refresh/` | 会影响留痕与审计 |
| 低 | `workspace/examples/` | 更多是演示，不应反向定义规则 |

### 4.3 目录维护红黄绿

| 颜色 | 含义 | 典型目录 |
|---|---|---|
| 绿 | AI 可高比例参与 | `knowledge/reference/`、`templates/`、`workspace/examples/` |
| 黄 | AI 可起草，但必须人复核 | `.opencode/commands/`、`.opencode/skills/`、`workspace/refresh/` |
| 红 | 必须人主导拍板 | `AGENTS.md`、`knowledge/rules/` 关键规则 |
...
|---|---|---|---|
| 阶段产物初稿 | `workspace/*/REQ-*/deliverable/`、`report/`、`handoff/`、`memory/` | 按模板产出初稿 | 事实、结论、是否通过门禁 |
| 审查报告初稿 | `*-review-report.md`、`stage-output-report.md` | 按标准结构汇总证据 | 审查结论是否成立 |
| 参考说明文档 | `knowledge/reference/*.md` | 做导航、表格、流程图、解释结构 | 真实经验、入口是否合理 |
| 模板初稿 | `templates/**/*.md` | 生成模板章节、表格字段、说明骨架 | 是否过度抽象、是否缺关键字段 |
| 索引清单文档 | `template-index.md`、各类 index 文档 | 汇总文件清单、阅读顺序、链接 | 是否漏链、是否误导 |
| 骨架修改日志初稿 | `workspace/refresh/skeleton-change-log.md` | 根据实际变更汇总影响 | 影响说明、后续动作是否准确 |
...
| 示例类文档 | `workspace/examples/` | 样例目录与示例内容 | 是否贴合真实项目 |
| 规则解释文档 | `knowledge/reference/*.md` | 条款说明、索引链接 | 例外边界、禁区、决策口径 |
...
|---|---|
| 最高层约束 | `AGENTS.md` |
| 治理规则 | `knowledge/rules/skeleton-change-governance.md` |
| 产物说明 | `knowledge/reference/stage-artifact-guide.md` |
| 责任清单 | `knowledge/reference/skeleton-artifact-ownership-guide.md` |
| 阶段产物布局 | `knowledge/rules/governance/stage-artifact-layout.md` |
| 结构入口 | `knowledge/reference/knowledge-structure.md`、`knowledge/reference/workspace-structure.md` |
| 加载 / 导航入口 | `knowledge/reference/skeleton-loading-matrix.md`、`knowledge/reference/team-onboarding-guide.md` |
| 模板导航 | `templates/template-index.md` |
| 映射总表 | `knowledge/reference/command-skill-artifact-map.md` |
| 变更留痕 | `workspace/refresh/skeleton-change-log.md` |
