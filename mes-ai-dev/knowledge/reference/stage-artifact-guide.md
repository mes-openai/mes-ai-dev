...
> 配套规则：`mes-ai-dev/knowledge/rules/governance/stage-artifact-layout.md`  
> 配套映射：`mes-ai-dev/knowledge/reference/command-skill-artifact-map.md`
...
> **先记 4 句：**
>
> 1. **`deliverable/` 和核心 `report/` 是优先看的。**
...
| 类型 | 定义 | 常见目录 | 是否必须长期保留 |
|---|---|---|---|
| 过程产物 | 为了推导、分析、校验、交接而产生的中间材料 | `working/`、部分 `report/`、部分 `evidence/` | 视治理需要保留 |
| 最终产物 | 作为阶段正式输出、下一阶段输入或正式交付依据的产物 | `deliverable/`、核心 `report/`、`handoff/` | 通常必须保留 |
...
| `report/` | 审查、汇总、门禁报告 | 最终产物为主 | 必看 |
| `evidence/` | 日志、截图、验证材料 | 过程产物为主 | 按需抽查 |
| `handoff/` | 阶段交接入口 | 最终产物 | 必看 |
| `memory/` | 坑点、决策、阻塞沉淀 | 过程与沉淀混合 | 建议看 |
...
| 需求分析 | `requirement-spec.md`、`spec-review-report.md` | `provider-selection.md`、`api-reuse-decision.md` | 边界与落点判断最关键 |
| 详细设计 | `design-doc.md`、`design-review-report.md` | `tech-approach.md`、`cross-service-consistency.md` | 设计收口不能只看中间稿 |
...
| `memory/` | 当作阶段交付物 | 它是经验沉淀和交接辅助 |
| 中间设计稿 | 替代 `design-doc.md` | 中间稿服务于主设计文档 |
...
1. `mes-ai-dev/knowledge/reference/command-skill-artifact-map.md`
2. `mes-ai-dev/knowledge/rules/governance/stage-artifact-layout.md`
3. `mes-ai-dev/knowledge/reference/workspace-structure.md`
4. `mes-ai-dev/knowledge/reference/skeleton-artifact-ownership-guide.md`
5. `mes-ai-dev/templates/template-index.md`
