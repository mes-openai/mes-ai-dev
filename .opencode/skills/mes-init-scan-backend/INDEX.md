# mes-init-scan-backend 索引

> 标题：后端目录扫描
> 描述：Scan backend directory structure and identify all microservice projects. Trigger keywords: scan backend, identify microservices, backend overview, init backend scan.
> 本索引只负责模块导航与使用优先级；目录职责、读取顺序与消费边界统一遵循骨架治理文档。

## 一、使用提醒
- 先读 `SKILL.md`，再进入当前任务命中的核心模块。
- 后端目录扫描主线优先读取执行步骤、输入、输出增强、输出、约束、审核与异常处理模块。
- 角色定位、目标仓适配与初始化补齐要求模块仅在当前任务需要时进入。
...
| 3 | `modules/module-07.md` | 必读 | 输出增强要求。 |
| 4 | `modules/module-08.md` | 必读 | 输出要求。 |
| 5 | `modules/module-09.md` | 必读 | 约束规则。 |
| 6 | `modules/module-10.md` | 必读 | 审核要点。 |
| 7 | `modules/module-11.md` | 必读 | 异常处理。 |
| 8 | `modules/module-01.md` ~ `modules/module-02.md` | 按需 | 角色定位、何时使用。 |
| 9 | `modules/module-05.md` ~ `modules/module-06.md` | 按需 | 目标仓结构适配要求、初始化补齐要求。 |

## 三、推荐最小读取集合

### 3.1 常规后端目录扫描
- `module-03` → `module-04` → `module-07` → `module-08` → `module-09` → `module-10` → `module-11`

### 3.2 需要补齐边界与仓适配时
- 按需追加：`module-01`、`module-02`、`module-05`、`module-06`
