# mes-deliver-acceptance-check 索引

> 标题：验收检查 Skill
> 描述：Verify deliverables against requirements. Trigger: acceptance, acceptance test, UAT, verification
> 本索引只负责模块导航与使用优先级；目录职责、读取顺序与消费边界统一遵循骨架治理文档。

## 一、使用提醒
- 先读 `SKILL.md`，再进入当前任务命中的核心模块。
- 验收检查主线优先读取输入依赖、输出产物、执行步骤与异常处理模块。
- 核心入口说明与审核检查清单模块仅在当前任务需要时进入。
...
| 6 | `modules/module-06.md` | 按需 | 审核检查清单。 |
