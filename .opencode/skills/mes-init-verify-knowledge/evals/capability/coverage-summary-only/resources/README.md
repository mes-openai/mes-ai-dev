# mes-init-verify-knowledge / coverage-summary-only 资源说明
...
## 观察点
- 该用例模拟的核心异常/风险是：state.yaml 只有 coverage_summary，没有对应 detail，且场景要求双写兼容。
...
## 期望结论
- 应指出 coverage 明细缺失并要求补齐。

## 回归判定提示
- 若未能命中该风险并输出“应指出 coverage 明细缺失并要求补齐。”，则说明该 Skill 对该类问题的回归覆盖仍不足。

## 补充说明
- 当前用例目标：验证 coverage 双写口径。
