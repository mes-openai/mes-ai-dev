# mes-init-scan-flows 索引

> 标题：业务链路与实体归属扫描 Skill
> 描述：Scan business flows, entity ownership and risk hotspots from codebase. Trigger: business flow, ownership, hotspot, critical path
> 本索引只负责模块导航与使用优先级；目录职责、读取顺序与消费边界统一遵循骨架治理文档。

## 一、使用提醒
- 先读 `SKILL.md`，再进入当前任务命中的核心模块。
- 业务链路与实体归属扫描主线优先读取执行步骤、输入、输出、约束与审核模块。
