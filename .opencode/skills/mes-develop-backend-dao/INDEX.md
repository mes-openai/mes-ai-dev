# mes-develop-backend-dao 索引

> 标题：后端DAO层开发 Skill
> 描述：Develop data access layer Mapper/Repository. Trigger: DAO, mapper, repository, MyBatis
> 本索引只负责模块导航与使用优先级；目录职责、读取顺序与消费边界统一遵循骨架治理文档。

## 一、使用提醒
- 先读 `SKILL.md`，再进入当前任务命中的核心模块。
- DAO 开发主线优先读取核心入口、MyBatis 真实性要求、执行步骤、输入、输出、约束与审核模块。
...
| 2 | `modules/module-05.md` | 必读 | MyBatis 真实性优先要求。 |
| 3 | `modules/module-07.md` | 必读 | 执行步骤主线（上半段）。 |
| 4 | `modules/module-08.md` | 必读 | 执行步骤主线（下半段）。 |
| 5 | `modules/module-09.md` | 必读 | 输入要求。 |
| 6 | `modules/module-10.md` | 必读 | 输出要求。 |
| 7 | `modules/module-11.md` | 必读 | 约束规则。 |
| 8 | `modules/module-12.md` | 必读 | 审核要点。 |

## 三、推荐最小读取集合

### 3.1 常规 DAO 开发
- `module-01` → `module-05` → `module-07` → `module-08` → `module-09` → `module-10` → `module-11` → `module-12`

### 3.2 需要重点复核 MyBatis 真实性时
- 在常规集合中重点复读：`module-05`
