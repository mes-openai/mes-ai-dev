# mes-init-extract-api 索引

> 标题：API端点提取
> 描述：Extract all Controller API endpoints from backend microservices. Trigger keywords: extract api, api registry, controller endpoints, rest api.
> 本索引只负责模块导航与使用优先级；目录职责、读取顺序与消费边界统一遵循骨架治理文档。

## 一、使用提醒
- 先读 `SKILL.md`，再进入当前任务命中的核心模块。
- API 端点提取主线优先读取执行步骤、输入、输出、约束、审核与异常处理模块。
- 角色定位与使用时机模块仅在当前任务需要时进入。
...
| 6 | `modules/module-08.md` | 必读 | 异常处理。 |
