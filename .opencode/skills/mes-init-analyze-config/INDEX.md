# mes-init-analyze-config 索引

> 标题：配置与依赖分析
> 描述：Analyze backend configuration and middleware dependencies. Trigger keywords: analyze config, middleware dependencies, service dependencies, restService.
> 本索引只负责模块导航与使用优先级；目录职责、读取顺序与消费边界统一遵循骨架治理文档。

## 一、使用提醒
- 先读 `SKILL.md`，再进入当前任务命中的核心模块。
- 配置与依赖分析主线优先读取执行步骤、输入、输出、约束、审核与异常处理模块。
- 角色定位、仓结构适配与初始化补齐要求模块仅在当前任务需要时进入。
...
### 3.1 常规配置与依赖分析
- `module-05` → `module-06` → `module-07` → `module-08` → `module-09` → `module-10` → `module-11`
