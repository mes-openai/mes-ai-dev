# mes-test-performance-analysis 索引

> 标题：性能分析 Skill
> 描述：Analyze performance of code changes and database queries. Trigger: performance, slow query, N+1, optimization
> 本索引只负责模块导航与使用优先级；目录职责、读取顺序与消费边界统一遵循骨架治理文档。

## 一、使用提醒
- 先读 `SKILL.md`，再进入当前任务命中的核心模块。
- 性能分析主线优先读取执行步骤、输入、输出、约束与审核模块。
- 角色定位、使用时机与性能分析报告模块仅在当前任务需要时进入。
...
| 9 | `modules/module-05.md` | 按需 | 性能分析报告。 |
