# mes-develop-db-migration 索引

> 标题：数据库迁移策略设计 Skill
> 描述：Design database migration strategy with rollback plan. Trigger: database migration, DDL migration, schema migration, rollback
> 本索引只负责模块导航与使用优先级；目录职责、读取顺序与消费边界统一遵循骨架治理文档。

## 一、使用提醒
- 先读 `SKILL.md`，再进入当前任务命中的核心模块。
- 数据库迁移策略设计主线优先读取执行步骤、输入、输出、约束与审核模块。
- 数据库迁移方案模块仅在当前任务需要时进入。
...
| 9 | `modules/module-05.md` | 按需 | 数据库迁移方案。 |
