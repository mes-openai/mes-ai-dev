# mes-develop-backend-service 索引

> 标题：后端Service层开发 Skill
> 描述：Develop business logic service layer. Trigger: service, business logic
> 本索引只负责模块导航与使用优先级；目录职责、读取顺序与消费边界统一遵循骨架治理文档。

## 一、使用提醒
- 先读 `SKILL.md`，再进入当前任务命中的核心模块。
- Service 开发主线优先读取核心入口、真实性要求、执行步骤、输入、输出、约束与审核模块。
...
| 2 | `modules/module-05.md` | 必读 | 真实性优先要求。 |
