...
└── .opencode/references/mes-ai-reference/templates/  # SDK 静态模板与代码骨架
        ...
        ├── governance/         # 治理与规范模板
        ├── code/               # 代码骨架模板（19个）
        ├── reference/          # 项目侧 reference 知识结构模板
        ├── rules/              # 项目侧 rules 知识结构模板
...
#### 1.5.2 知识库最终共享产物（`knowledge/` 下）

| 路径模式 | 产物类型 | 过程/最终 | 默认维护方式 | 主要生成阶段/命令 | 典型维护动作 | 最终使用者 |
|------|------|---------|------------|------------------|-------------|-----------|
| `knowledge/code-map/backend-overview.md` | 后端总览 | 最终产物 | AI主生成 + 主控收口 | `/mes-init-project`、`/mes-init-converge` | 触发初始化/收敛生成；人工只做抽查纠偏 | 所有阶段的第0层入口、项目经理、架构师、AI |
| `knowledge/code-map/frontend-overview.md` | 前端总览 | 最终产物 | AI主生成 + 主控收口 | `/mes-init-project`、`/mes-init-converge` | 同上 | 所有阶段、前端负责人、AI |
| `knowledge/code-map/services/service-xxx/index.md` | 服务精简索引 | 最终产物 | AI主生成 | `/mes-init-project`、`/mes-init-enrich` | 通过初始化/深化刷新，不建议人工手改正文 | 需求分析、设计、AI |
| `knowledge/code-map/services/service-xxx/detail.md` | 服务完整详情 | 最终产物 | AI主生成 | `/mes-init-enrich` | 由 AI 深化生成，人工按需纠偏 | 设计、开发、AI |
| `knowledge/code-map/services/service-xxx/file-summaries.md` | 文件摘要 | 最终产物 | AI主生成 | `/mes-init-enrich` | 由 AI 按代码扫描生成 | 开发、AI |
| `knowledge/code-map/services/service-xxx/repo-profile.md` | 仓库画像 | 最终产物 | AI首稿 + 人工校正 | `/mes-init-enrich` 或专项补齐 | 可直接改文件，也可先给 AI 提示再归档 | 设计、开发、测试、AI |
| `knowledge/code-map/modules/module-xxx/index.md` | 模块精简索引 | 最终产物 | AI主生成 | `/mes-init-project`、`/mes-init-enrich` | 通过初始化/深化刷新 | 需求分析、设计、AI |
| `knowledge/code-map/modules/module-xxx/detail.md` | 模块完整详情 | 最终产物 | AI主生成 | `/mes-init-enrich` | 通过深化刷新 | 设计、开发、AI |
| `knowledge/code-map/modules/module-xxx/file-summaries.md` | 模块文件摘要 | 最终产物 | AI主生成 | `/mes-init-enrich` | 通过深化刷新 | 开发、AI |
| `knowledge/code-map/business-flows.md` | 业务链路 | 最终产物 | AI主生成 + 主控收口 | `/mes-init-enrich`、`/mes-init-converge` | 可由 AI 先写片段，主控收口；人工做链路校验 | 需求分析、测试、AI |
| `knowledge/code-map/ownership.md` | 实体归属与事实源 | 最终产物 | AI主生成 + 人工校正 | `/mes-init-enrich`、`/mes-init-converge` | 人工重点确认事实源与高风险归属 | 需求分析、设计、开发、AI |
| `knowledge/code-map/patterns.md` | 实现模式目录 | 最终产物 | **AI首稿 + 人工校正** | `/mes-init-enrich`、整改专项 | 可直接改正式文件；也可先让 AI 按模板产出再人工补真实参考文件 | 设计、开发、评审、AI |
| `knowledge/code-map/business-rules.md` | 真实业务规则库 | 最终产物 | **AI首稿 + 人工校正** | `/mes-init-enrich`、整改专项 | 允许直接改正式文件；推荐“人给要点/证据 → AI归档成文 → 人复核” | 需求分析、设计、开发、测试、AI |
| `knowledge/code-map/change-impact-memory.md` | 变更影响记忆 | 最终产物 | **人主补充 + AI辅助整理** | 整改专项、后续 refresh / 复盘回补 | 可直接改正式文件；推荐由人提供历史经验，再由 AI 结构化整理 | 需求分析、设计、开发、测试、交付、AI |
| `knowledge/code-map/test-assets.md` | 测试资产索引 | 最终产物 | AI首稿 + 人工校正 | `/mes-init-enrich`、测试阶段补齐 | AI先扫测试资产，人补手工回归入口 | 测试、开发、AI |
| `knowledge/code-map/runtime.md` | 运行时配置地图 | 最终产物 | AI首稿 + 人工校正 | `/mes-init-enrich`、交付/运维补齐 | AI先抽配置，人补环境差异与功能开关语义 | 设计、测试、交付、运维、AI |
| `knowledge/code-map/hot-services.md` | 高频服务排行 | 最终产物 | AI主生成 + 主控重算 | `/mes-init-enrich`、`/mes-init-converge` | 不建议人工直接手改排行结果 | 大仓需求分析、AI |
| `knowledge/code-map/hot-apis.md` | 高频API排行 | 最终产物 | AI主生成 + 主控重算 | `/mes-init-enrich`、`/mes-init-converge` | 同上 | 大仓需求分析、AI |
| `knowledge/code-map/hot-tables.md` | 高频表排行 | 最终产物 | AI主生成 + 主控重算 | `/mes-init-enrich`、`/mes-init-converge` | 同上 | 设计、测试、AI |
| `knowledge/code-map/legacy-debt.md` | 存量仓历史债务 | 最终产物 | **AI首稿 + 人工校正** | `/mes-init-enrich`、整改专项 | 允许直接改正式文件；人应重点确认禁区/高风险区/受控修改区 | 开发、紧急修复、架构师、AI |
| `knowledge/code-map/testability-matrix.md` | 可测性矩阵 | 最终产物 | AI首稿 + 人工校正 | `/mes-init-enrich`、测试阶段补齐 | AI先评分，人补真实难点 | 测试、设计、AI |
| `knowledge/code-map/e2e-chains.md` | E2E链路 | 最终产物 | AI首稿 + 人工校正 | `/mes-init-enrich`、测试阶段补齐 | AI先梳链，人确认关键断点与验证点 | 测试、交付、AI |
| `knowledge/dependency-graph/service-dependencies.md` | 服务依赖图 | 最终产物 | AI主生成 + 主控收口 | `/mes-init-project`、`/mes-init-converge` | 主控汇总，人工按需纠偏 | 需求分析、设计、AI |
| `knowledge/dependency-graph/api-registry.md` | API注册表 | 最终产物 | AI主生成 + 主控收口 | `/mes-init-project`、`/mes-init-converge` | 主控汇总，必要时索引化拆分 | 需求分析、设计、开发、AI |
| `knowledge/dependency-graph/database-registry.md` | 数据库注册表 | 最终产物 | AI主生成 + 主控收口 | `/mes-init-project`、`/mes-init-converge` | 主控汇总 | 设计、测试、AI |
| `knowledge/dependency-graph/frontend-backend-map.md` | 前后端映射 | 最终产物 | AI主生成 + 主控收口 | `/mes-init-project`、`/mes-init-converge` | 主控汇总 | 需求分析、测试、AI |
| `knowledge/database-index/schema-xxx/index.md` | Schema索引 | 最终产物 | AI主生成 | `/mes-init-project`、`/mes-init-enrich` | 初始化/深化生成，不建议手工长期维护 | 设计、测试、AI |
...
| `knowledge/state/state.yaml` | 唯一机器事实源 | 最终产物 | AI主写入 + 人工审语义 | 初始化/收敛/刷新/治理修改 | 不建议手工随意改值；如需改结构，先审规则 | 所有阶段、AI |
| `knowledge/state/summary.md` | 人工摘要入口 | 最终产物（兼容视图） | AI主渲染 | `/mes-init-converge`、`/mes-refresh-knowledge` | 原则上不直接人工维护正文 | 人工查看、AI回退消费 |
| `knowledge/baseline.md` | 兼容摘要视图 | 最终产物（兼容视图） | AI主渲染 | `/mes-init-converge`、`/mes-refresh-knowledge` | 不直接人工维护正文 | Phase 0 预检、人工查看、AI回退 |
| `knowledge/init-coverage.md` | 兼容清单视图 | 最终产物（兼容视图） | AI主渲染 | `/mes-init-converge`、`/mes-refresh-knowledge` | 不直接人工维护正文 | 覆盖率查看、AI回退 |

#### 1.5.3 初始化/深化/刷新过程产物（只允许过程消费，不得直接当正式事实源）

| 路径模式 | 产物类型 | 过程/最终 | 默认维护方式 | 主要生成阶段/命令 | 典型维护动作 | 最终使用者 |
|------|------|---------|------------|------------------|-------------|-----------|
| `knowledge/state/fragments/<scope-type>-<scope-name>.yaml` | 状态片段 | 过程产物 | AI主生成 | `/mes-init-project`、`/mes-init-enrich` | 不直接人工编辑，异常时按例外流程处理 | 仅主控 Agent / `/mes-init-converge` |
| `knowledge/fragments/reference/**/*.md` | reference 局部片段 | 过程产物 | AI主生成 | `/mes-init-extract-reference`、`/mes-init-enrich` | 先写片段，再由主控收口；人工可审核不直接下游消费 | 仅主控 Agent / `/mes-init-converge` |
| `knowledge/fragments/rules/**/*.md` | 规则片段 | 过程产物 | AI主生成 | `/mes-init-extract-reference`、`/mes-init-enrich` | 同上 | 仅主控 Agent / `/mes-init-converge` |
| `knowledge/fragments/code-map/**/*.md` | code-map 局部片段 / 热点候选 | 过程产物 | AI主生成 | `/mes-init-scan-flows`、`/mes-init-scan-patterns`、`/mes-init-scan-hotspots`、`/mes-init-enrich` | 同上 | 仅主控 Agent / `/mes-init-converge` |
...
> **重要限制**：`knowledge/state/fragments/*.yaml` 与 `knowledge/fragments/**/*.md` 都是过程产物，**不能直接给下游阶段当正式知识消费**。必须经过 `/mes-init-converge` 或主控收口。
...
| 输出 | 说明 |
|------|------|
| `knowledge/code-map/backend-overview.md` | 后端总览（所有服务列表、用途、规模） |
| `knowledge/code-map/frontend-overview.md` | 前端总览（所有模块列表、用途、规模） |
| `knowledge/code-map/services/service-xxx/index.md` | 各服务精简索引 |
| `knowledge/code-map/services/service-xxx/detail.md` | 各服务完整详情 |
| `knowledge/code-map/services/service-xxx/file-summaries.md` | 各服务文件摘要 |
| `knowledge/code-map/modules/module-xxx/index.md` | 各模块精简索引 |
| `knowledge/code-map/modules/module-xxx/detail.md` | 各模块完整详情 |
| `knowledge/code-map/modules/module-xxx/file-summaries.md` | 各模块文件摘要 |
| `knowledge/dependency-graph/api-registry.md` | API注册表（5000+ API清单） |
| `knowledge/dependency-graph/service-dependencies.md` | 服务间调用关系 |
| `knowledge/dependency-graph/database-registry.md` | 数据库归属映射 |
| `knowledge/dependency-graph/frontend-backend-map.md` | 前后端映射 |
| `knowledge/database-index/schema-xxx/` | 数据库表结构索引 |
...
| `mes-init-index-database` | 索引数据库脚本 | dbscript/路径 | database-index/ |
...
| `mes-init-build-dependency-graph` | 生成依赖关系图 | 以上全部 | dependency-graph/ |
...
2. 手动清理 `mes-ai-dev/knowledge/code-map/`、`state/`、`mes-ai-dev/workspace/各阶段目录`
3. 修改 `AGENTS.md` 中的配置
