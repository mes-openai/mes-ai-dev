...
related_files:
  - .opencode/references/mes-ai-reference/reference/status-tracker-governance-summary.md
  - .opencode/references/mes-ai-reference/rules/scenarios/scenario-state-migration.md
---

# 状态追踪治理规范

> 对应摘要：`.opencode/references/mes-ai-reference/reference/status-tracker-governance-summary.md`
> 若当前不涉及状态更新、迁移、闭环追踪或门禁联动校验，优先读取摘要并禁止默认整篇读取正文。
> 本文件定义 `mes-ai-dev/workspace/status-tracker.md` 的字段含义、状态流转规则，以及与阶段门禁、步骤级门禁的联动关系。

---

## 一、定位

- `mes-ai-dev/workspace/status-tracker.md` 是需求全生命周期的统一状态总表
- 结构模板：`.opencode/references/mes-ai-reference/templates/governance/status-tracker-template.md`
...
| 代码开发 | `mes-ai-dev/workspace/development/{REQ-ID}/` |
