...
- 已检查编码前思考结果，包含初始化 scope、仓/模块/Schema 边界、产物层级、断点策略与未知项标记方式
- 已检查简洁优先，未为当前 scope 生成无关深度内容或全量长文档
- 已检查精准修改，单仓/定向初始化未越界扫描、越界写入或覆盖非当前 scope 共享知识
- 已检查目标驱动执行，知识面覆盖、局部产物、状态片段和下游消费映射可验证
- 若使用 GitNexus 类能力，已说明其辅助抽取的结构关系、调用链、依赖链或图谱查询入口，并区分确认/候选/未知
- 若使用 graphify 类能力，已说明图谱报告或 wiki 导读不替代初始化正式产物和三态结论
