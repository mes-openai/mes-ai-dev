...
| 文件 | 角色 | 消费场景 |
|---|---|---|
| `mes-ai-dev/knowledge/state/state.yaml` | 唯一已合并机器事实源 | 初始化、深化、收敛、刷新、后续消费类阶段 |
| `mes-ai-dev/knowledge/state/state-index.md` | 状态结构导航入口 | 判断主文件与 detail 文件的消费边界 |
| `mes-ai-dev/knowledge/state/state-detail/*.yaml` | 状态细节文件（迁移目标结构） | 主文件不足时按需下钻 |
| `mes-ai-dev/knowledge/state/fragments/*.yaml` | 初始化运行中状态片段 | 初始化、深化、断点续传 |
| `mes-ai-dev/knowledge/state/summary.md` | 人工摘要入口 | 快速查看状态结论 |
...
1. 正式事实判断默认读取 `mes-ai-dev/knowledge/state/state.yaml`
2. 若 `state.yaml` 未来完成轻量化，状态细节应迁移到 `mes-ai-dev/knowledge/state/state-detail/*.yaml`，消费顺序为“先主文件，再按需下钻 detail”
3. `state-index.md` 只负责导航主文件与 detail 文件结构，不替代 `state.yaml`
4. `mes-ai-dev/knowledge/state/fragments/*.yaml` 仅作为初始化/深化阶段的待合并片段，不得作为下游正式事实源
