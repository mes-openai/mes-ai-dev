...
> 详细设计步骤、关键对象展开要求与技能入口见：`.opencode/references/mes-ai-reference/rules/phases/phase-design-detail.md`
...
- `.opencode/references/mes-ai-reference/reference/parameter-switch-patterns.md`
...
- `.opencode/references/mes-ai-reference/reference/knowledge-consumption/reference.md`
- `.opencode/references/mes-ai-reference/reference/knowledge-consumption/dependency-graph.md`
- `.opencode/references/mes-ai-reference/reference/parameter-switch-patterns.md`（命中参数开关/配置取值场景时必读）
...
- **编码前思考**：设计前必须列明约束、备选方案、取舍理由、兼容性边界、回退边界与仍需确认的问题。
- **简洁优先**：优先采用最小可行设计和现有模式复用，不得为单一需求引入不必要的抽象层、平行机制或通用平台化设计。
- **精准修改**：设计范围只覆盖当前需求确认的仓、服务、表、接口、页面、配置与脚本，不得扩展到未授权重构。
- **目标驱动执行**：每个关键设计对象必须能映射到开发任务、测试对象和验收标准。

命中跨服务调用、Provider 决策、接口复用、重构风险或影响面不清时，可按需使用 GitNexus 类代码知识图谱能力校验：
...
命中复杂设计文档、跨模块关系或交接阅读成本较高时，可按需使用 graphify 类能力为 `design.md` 补充关系导读、图谱报告或 wiki 化索引。graphify 输出不得替代设计正文，所有设计结论仍必须在阶段主文档中明确写出。
...
- 不得省略知识来源标注
- 不得在未证明必要性的情况下引入超出当前目标的复杂抽象或平行机制
- 不得把 GitNexus / graphify 的图谱推断直接替代服务链冻结、Provider 选择和私有契约事实源
...
## 八、详版入口
- 详细设计步骤、关键对象展开要求与推荐技能入口：`.opencode/references/mes-ai-reference/rules/phases/phase-design-detail.md`
