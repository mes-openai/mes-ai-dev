...
> 详细执行步骤、扩展说明与技能入口见：`.opencode/references/mes-ai-reference/rules/phases/phase-develop-detail.md`
...
### 默认不加载
- `.opencode/references/mes-ai-reference/reference/phase-gates.md` 旧总表全文
- 非当前阶段门禁分片
...
- `.opencode/references/mes-ai-reference/reference/phase-gates/develop.md`（按需）
- `.opencode/references/mes-ai-reference/reference/knowledge-consumption/rules.md`（按需）
- `.opencode/references/mes-ai-reference/reference/knowledge-consumption/stage-memory.md`（按需）
- `.opencode/references/mes-ai-reference/rules/governance/review-report-standard.md`
- `.opencode/references/mes-ai-reference/rules/governance/stage-artifact-layout.md`
- `.opencode/references/mes-ai-reference/rules/governance/completion-sweep-standard.md`
...
- **编码前思考**：动手前必须确认设计结论、任务边界、现有模式、验证目标和风险点；发现边界不清时先回流，不得边猜边写。
- **简洁优先**：优先最小必要实现，复用既有分层、工具类、接口和测试组织，不为局部需求制造新抽象。
- **精准修改**：只修改当前任务直接需要的文件和测试，不做顺手格式化、无关重命名、无关重构或跨模块清理。
- **目标驱动执行**：以编译、测试、覆盖率、TDD 闭环、自审和阶段门禁作为完成标准。

命中目标文件不清、调用链复杂、影响面不明、重命名/重构风险或回归范围需要确认时，可按需使用 GitNexus 类代码知识图谱能力辅助定位：
...
GitNexus 图谱结果只能帮助缩小和验证改动范围，不得成为扩大编辑范围的理由；若图谱提示的影响面超出设计边界，必须回流 analyze/design。
...
6. 新生成测试用例通过后，必须复核本轮生成/修改并纳入验证范围的行覆盖率、分支覆盖率、方法覆盖率是否均达到 100%
...
- 不得进行无关格式化、无关重命名、无关重构或“顺手优化”
- 不得把图谱分析发现的非目标问题混入当前开发任务，除非用户确认扩大范围
...
## 八、详版入口
- 详细执行步骤、补强要求与推荐技能入口：`.opencode/references/mes-ai-reference/rules/phases/phase-develop-detail.md`
